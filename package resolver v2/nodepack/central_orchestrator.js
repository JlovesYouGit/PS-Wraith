...
class PythonSubBridge {
    constructor(workDir, nodepackDir, flashMode = null) {
        this.workDir = workDir;
        ...
        return new Promise((resolve, reject) => {
            ...
            this.process.stdout.on('data', (data) => {
                ...
                if (msg.includes('[PYTHON_BRIDGE_READY]')) {
                    ...
                    if (this.flashMode && this.flashMode.isFlashMode()) {
                        console.log('[Node] Flash mode active for Python bridge');
                    }
                    resolve(true);
                    return;
                }
...
class CentralOrchestrator {
    ...
    async initialize() {
        ...
        if (this.flashMode && this.flashMode.isFlashMode()) {
            console.log('[FLASH] Wraith flash mode initialized');
            const boostData = this.flashMode.runWraithBoost();
            if (boostData) {
                console.log(`[FLASH] Gramshas Engine: ACTIVE`);
            ...
            console.log(`[FLASH] Mirror initialized: ${mirrorStatus.deviceConnected ? 'DEVICE CONNECTED' : 'NO DEVICE'}`);

            // Initialize GoldHEN if available
            if (this.flashMode.isGoldHENAvailable()) {
                console.log('[FLASH] GoldHEN integration available');
                console.log(`[FLASH] GoldHEN directory: ${this.flashMode.goldhenDir}`);
            }
        }
    ...
    async runNodeResolver() {
        ...
        const args = [resolverPath, this.workDir];
...
async function main() {
    ...
    const mode = process.argv[3] || 'run'; // run, loop, flash
    ...
    if (mode === 'flash') {
        ...
        if (!flashStatus.active) {
            console.log('Failed to activate flash mode - not in wraith directory');
            process.exit(1);
        }

        // Run device image bridge
        try {
            await flashMode.runDeviceImageBridge();
        } catch (e) {
        ...
        try {
            ...
            console.log('[Flash] Mirror status:', mirrorStatus);
        } catch (e) {
            console.log('[Flash] Mirror sync note:', e.message);
        }
    }

    const orchestrator = new CentralOrchestrator(workDir, flashMode);
    ...
    if (mode === 'loop') {
        ...
        console.log('\nFlash mode operation complete');
        console.log('Run with "loop" arg for continuous operation:');
        console.log('  node central_orchestrator.js <dir> loop');
        orchestrator.stop();
    } else {
        console.log('\nRun complete. Use "loop" mode for continuous operation:');
        console.log('  node central_orchestrator.js <dir> loop');
        console.log('Use "flash" mode for wraith directory operation:');
        console.log('  node central_orchestrator.js <dir> flash');
        orchestrator.stop();
    }
}
