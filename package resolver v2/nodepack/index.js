...
class UniversalPackageResolver {
    constructor(parserDir, flashMode = null) {
    ...
    async resolve(workDir) {
        if (this.flashMode && this.flashMode.isFlashMode()) {
            return this.resolveFlash(workDir);
        }
    ...
    // Flash mode resolution - fast track for wraith directory
    async resolveFlash(workDir) {
        console.log('[FLASH] Running flash mode resolution...');
        
        const resolverScript = path.join(this.parserDir, 'resolver.js');
        const flashConfig = this.flashMode.getFlashResolutionConfig();
        
        return new Promise((resolve, reject) => {
            const proc = spawn('node', [resolverScript, workDir, 'adjust'], {
                stdio: ['pipe', 'pipe', 'pipe']
            });
            ...
            proc.stderr.on('data', (data) => {
                const msg = data.toString().trim();
                if (msg) console.log('[FlashResolver]', msg);
            });
            ...
            proc.on('close', (code) => {
                try {
                    ...
                    if (result.status === 'success') {
                        result.flash_mode = true;
                        result.wraith_integration = true;
                        result.engrave_layers = flashConfig.engraveLayers;
                        result.rj45_active = flashConfig.rj45Active;
                        this.activeColumn = result;
                        this.resolvedPackages = result.packages || [];
                        this.loadColumnStore(workDir);
                    }
                    ...
                    reject(new Error(`Flash parse error: ${e.message}, output: ${output}`));
                }
            });
        });
    }
    ...
    async installLanguagePackages(language, workDir) {
        ...
        if (this.flashMode && this.flashMode.isFlashMode()) {
            console.log(`\n[FLASH] Installing ${packages.length} ${language} packages in wraith mode...`);
        } else {
...
async function main() {
    ...
    const mode = process.argv[3] || 'auto'; // 'auto', 'terminal', 'watch', 'flash'
    const parserDir = path.join(__dirname, 'library parser logic');
    
    let flashMode = null;
    
    // Initialize flash mode if requested
    if (mode === 'flash') {
        flashMode = new WraithFlashMode(workDir);
        const flashStatus = flashMode.activateFlash();
        if (!flashStatus.active) {
            console.log('Failed to activate flash mode');
            process.exit(1);
        }
        console.log('[FLASH] Operating in wraith directory flash mode');
    }
    ...
    if (flashMode) {
        console.log('Flash mode: ACTIVE');
        console.log('Wraith directory:', flashMode.wraithDir);
    }
    
    // Initialize resolver
    const resolver = new UniversalPackageResolver(parserDir, flashMode);
    ...
    if (mode === 'terminal' || mode === 'interactive') {
        ...
        // Flash mode - wraith directory optimized operation
        console.log('\n=== Flash Mode: Wraith Directory Operation ===');
        
        if (flashMode) {
            const boostData = flashMode.runWraithBoost();
            if (boostData) {
                console.log('[FLASH] Wraith boost data integrated');
                console.log(`[FLASH] Gramshas Engine: ${boostData.gramshasEngine ? 'ACTIVE' : 'STANDBY'}`);
                console.log(`[FLASH] Disk Layer: ${boostData.diskLayer}`);
                console.log(`[FLASH] Performance Hash: ${boostData.performanceHash}`);
            }

            // Run device image bridge
            try {
                const imageResult = await flashMode.runDeviceImageBridge();
                if (imageResult) {
                    console.log('[FLASH] Device image bridge initialized');
                }
            } catch (e) {
                console.log('[FLASH] Image bridge note:', e.message);
            }

            // Run mirror sync
            try {
                const mirrorStatus = await flashMode.runMirrorSync();
                console.log('[FLASH] Mirror sync complete');
                console.log(`[FLASH] Mirror packages: ${mirrorStatus.packageCount}`);
                console.log(`[FLASH] Mirror images: ${mirrorStatus.imageCount}`);
            } catch (e) {
                console.log('[FLASH] Mirror sync note:', e.message);
            }

            // Run IOsrom flash if available
            if (flashMode.isIOsromAvailable()) {
                console.log('[FLASH] IOsrom integration available');
                console.log(`[FLASH] IOsrom directory: ${flashMode.iosromDir}`);
            }

            // Run GoldHEN deploy if available
            if (flashMode.isGoldHENAvailable()) {
                console.log('[FLASH] GoldHEN integration available');
                console.log(`[FLASH] GoldHEN directory: ${flashMode.goldhenDir}`);
                try {
                    const goldhenResult = await flashMode.runGoldHENDeploy();
                    if (goldhenResult) {
                        console.log('[FLASH] GoldHEN deployment complete');
                    }
                } catch (e) {
                    console.log('[FLASH] GoldHEN deploy note:', e.message);
                }
            }
        }
        ...
        console.log('\n✓ Flash mode resolution complete');
    } else {
