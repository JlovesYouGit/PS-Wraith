...
class PythonEnvironmentBridge:
    ...
    Wraith Flash Mode: Direct integration with wraithps4-serv-backup-performance booster
    """
    
    def __init__(self, work_dir: str, nodepack_dir: str, flash_mode: bool = False):
        ...
        self.rj45_connected = False
        
    def _detect_wraith_directory(self) -> Optional[str]:
        """Detect if operating in wraithps4-serv-backup-performance booster directory"""
        wraith_indicators = [
            'boost_engine.py',
        ...
        parent_dir = os.path.join(self.work_dir, '..')
        for file in wraith_indicators:
            if os.path.exists(os.path.join(parent_dir, file)):
                return parent_dir
    ...
    def initialize(self):
        ...
        if self.flash_mode and self.wraith_dir:
            self._initialize_wraith_bridge()
        ...
        self._send_message({
            ...
            'wraith_dir': self.wraith_dir
        })
        
    def _initialize_wraith_bridge(self):
        """Initialize wraith directory bridge connection"""
        if not self.wraith_dir:
            return
            
        sys.path.insert(0, self.wraith_dir)
        ...
        try:
            ...
            self.boost_engine = GramshasEngine()
            self.hardware_interface = WraithHardwareInterface()
            self.pragma_logic = WraithPragmaLogic()
            self.rj45_connected = self.hardware_interface.detect_rj45_device()
            
            self._send_message({
                'type': 'wraith_bridge_initialized',
                'wraith_dir': self.wraith_dir,
                'rj45_connected': self.rj45_connected
            })
        except ImportError as e:
            self._send_message({
                'type': 'wraith_bridge_error',
                'error': str(e)
            })
...
def main():
    if len(sys.argv) < 3:
        print("Usage: python_subbridge.py <work_dir> <nodepack_dir> [flash_mode]", file=sys.stderr)
        sys.exit(1)
    
    work_dir = sys.argv[1]
    nodepack_dir = sys.argv[2]
    flash_mode = sys.argv[3] == 'flash' if len(sys.argv) > 3 else False
    
    bridge = PythonEnvironmentBridge(work_dir, nodepack_dir, flash_mode)
    bridge.run()
